...
def extract_waste(image_path):
    img = cv2.imread(image_path, cv2.IMREAD_GRAYSCALE)
    img_blur = cv2.medianBlur(img, 5)
    waste = img.astype(np.int16) - img_blur.astype(np.int16)
    visual_waste = np.clip(waste + 128, 0, 255).astype(np.uint8)
    eq_waste = cv2.equalizeHist(visual_waste)
    colored_waste = cv2.applyColorMap(eq_waste, cv2.COLORMAP_MAGMA)

    return colored_waste
